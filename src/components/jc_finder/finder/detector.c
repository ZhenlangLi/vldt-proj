    ...
    for(jab_int32 k = 0; k<number; k++)
    {
        if(fps[k].found_count > 0)
        {
            ...
            for(jab_int32 i=starty; i<endy; i++)
            {
                ...
                {
                    reportError(("Drawing finder pattern %d out of image", k))
                    break;
                }
            }
...
Int32 selectBestPatterns(J_Finder_Pattern* fps, Int32 fp_count, Int32* fp_type_count)
{
    ...
    J_Finder_Pattern fps0[fp_type_count[FP0]];
    J_Finder_Pattern fps1[fp_type_count[FP1]];
    J_Finder_Pattern fps2[fp_type_count[FP2]];
    J_Finder_Pattern fps3[fp_type_count[FP3]];
...
void seekMissingFinderPattern(Bitmap* bitmap, J_Finder_Pattern* fps, Int32 miss_fp_index)
{
    ...
    for(Int32 i=0; i<3; i++)
    {
        ...
        if(rgb[i] == NULL)
        {
            reportError(("Memory allocation for binary bitmap failed, the missing finder pattern can not be found."));
            return;
        }
 ...
 */
Float crossCheckPatternDiagonalAP(Bitmap* image, Int32 ap_type, Int32 module_size_max, POINT center, Int32* dir)
{
    Int32 offset_x, offset_y;
    Boolean fix_dir = 0;
    //if the direction is given, ONLY check the given direction
 ...
 */
Float crossCheckPatternVerticalAP(Bitmap* image, POINT center, Int32 module_size_max, Float* module_size)
{
    Int32 state_count[3] = {0};
...
Int32 detectFirstAP(Bitmap* ch[], Int32 side_version, J_Finder_Pattern fp1, J_Finder_Pattern fp2)
{
    ...
    {
        //distance to FP1
        Float distance = fp1.module_size * (J_AP_POSITION[next_version-1][1] - J_AP_POSITION[next_version-1][0]);
...
Int32 confirmSideVersion(Int32 side_version, Int32 first_ap_pos)
{
    ...
    {
#if TEST_MODE
        reportError(("Invalid position of the first AP."))
#endif
        return FAILURE;
    }
    ...
    do
    {
        if(first_ap_pos == J_AP_POSITION[v-1][1])
        {
            flag = 1;
            break;
        }
...
Int32 confirmSymbolSize(Bitmap* ch[], J_Finder_Pattern* fps, J_Decoded_Symbol* symbol) {
    ...
    if(side_version_x == 0) //if failed, try the line between FP3 and FP2
    {
        ...
        {
#if TEST_MODE
            reportError(("Confirming side version x failed."))
#endif
            return FAILURE;
        }
    }
    ...
    if(side_version_y == 0) //if failed, try the line between FP1 and FP2
    {
        ...
        {
#if TEST_MODE
            reportError(("Confirming side version y failed."))
#endif
            return FAILURE;
        }
    }
...
J_Found_Symbol* detectMaster( Bitmap* bitmap, Bitmap* ch[]) {
    ...
    if(fps == NULL) {
        reportError("Can't find master symbol, abort");
        return NULL;
    }
    ...
    J_Found_Symbol* master_symbol = malloc(sizeof(J_Found_Symbol));
    if (!master_symbol) {
        reportError("Memory of master symbol found data allocation failed");
        free(fps);
        return NULL;
    }
