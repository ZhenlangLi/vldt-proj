...
Bitmap* sampleSymbolByAlignmentPattern(Bitmap* bitmap, Bitmap* ch[], J_Decoded_Symbol* symbol, J_Finder_Pattern* fps) {
		...
		for(Int32 j = 0; j < number_of_ap_x; j++) {
			...
			if (i == 0 && j == 0)
				...
				if(i == 0) {
					...
					Float distance = aps[j-1].module_size * (J_AP_POSITION[side_ver_x_index][j] - J_AP_POSITION[side_ver_x_index][j-1]);
					...
					Float distance = aps[(i-1) * number_of_ap_x].module_size * (J_AP_POSITION[side_ver_y_index][i] - J_AP_POSITION[side_ver_y_index][i-1]);
					//calculate the coordinate of ap[index]
	...
	for(Int32 i=0; i<rect_index; i+=2) {
		...
		//middle blocks
		blk_size.x = J_AP_POSITION[side_ver_x_index][rect[i+1].x] - J_AP_POSITION[side_ver_x_index][rect[i].x] + 1;
		blk_size.y = J_AP_POSITION[side_ver_y_index][rect[i+1].y] - J_AP_POSITION[side_ver_y_index][rect[i].y] + 1;
		p0.x = 0.5f;
		p0.y = 0.5f;
		...
		Int32 start_x = J_AP_POSITION[side_ver_x_index][rect[i].x] - 1;
		Int32 start_y = J_AP_POSITION[side_ver_y_index][rect[i].y] - 1;
		if(rect[i].x == 0)
			start_x = 0;
		if(rect[i].y == 0)
			start_y = 0;
