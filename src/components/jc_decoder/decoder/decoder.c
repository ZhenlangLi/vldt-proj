...
// Han Li adapted from original work of Fraunhofer SIT
...
/**
 * @brief Write colours into colour palettes
 * @param matrix the symbol matrix
 * @param symbol the master/slave symbol
 * @param p_index the colour palette index
 * @param colour_index the colour index in colour palette
 * @param x the x coordinate of the colour module
 * @param y the y coordinate of the colour module
 */
...
/**
 * @brief Get the coordinates of the modules in finder/alignment patterns used for colour palette
 * @param p_index the colour palette index
...
/**
 * @brief Read the colour palettes in master symbol
 * @param matrix the symbol matrix
...
Int32 readColourPaletteInMaster(Bitmap* matrix, J_Decoded_Symbol* symbol, BYTE* data_map, Int32* module_count, Int32* x, Int32* y) {
    ...
    //read colours from finder patterns
...
#if TEST_MODE
            reportError("Invalid module colour in primary metadata part 1 found");
#endif
...
#if TEST_MODE
        reportError("Invalid colour combination in primary metadata part 1 found");
#endif
...
J_Data* readRawModuleData(Bitmap* matrix, J_Decoded_Symbol* symbol, BYTE* data_map, Float* norm_palette, Float* pal_ths) {
...
#if TEST_MODE
    jab_byte decoded_module_colour_index[matrix->height * matrix->width];
#endif
...
#if TEST_MODE
    // 仅供测试：将读取出的二进制写入一个文件中
    FILE* fp1 = fopen("j_dec_module_sampled_rgb.raw", "wb");
    FILE* fp2 = fopen("j_dec_module_decoded_rgb.raw", "wb");
    for(jab_int32 i=0; i<matrix->height; i++) {
        for(jab_int32 j=0; j<matrix->width; j++) {
            ...
            if(data_map[i*matrix->width + j] == 0) {
                jab_int32 index = decoded_module_colour_index[i*matrix->width + j];
 ...
 * @return SUCCESS | FAILURE | DECODE_METADATA_FAILED | FATAL_ERROR
 */
Int32 decodeSymbol(Bitmap* matrix, J_Decoded_Symbol* symbol, BYTE* data_map, Float* norm_palette, Float* pal_ths, Int32 type) {
#if TEST_MODE
    jab_int32 colour_number = (jab_int32)pow(2, symbol->metadata.Nc + 1);
    printf("p1:\n");
    ...
    printf("p2:\n");
    for(int i=0;i<colour_number;i++) {
        printf("%d\t%d\t%d\n", symbol->palette[3*colour_number+i*3], symbol->palette[3*colour_number+i*3+1], symbol->palette[3*colour_number+i*3+2]);
    }
    printf("p3:\n");
    for(int i=0;i<colour_number;i++) {
        printf("%d\t%d\t%d\n", symbol->palette[3*colour_number*2+i*3], symbol->palette[3*colour_number*2+i*3+1], symbol->palette[3*colour_number*2+i*3+2]);
    }
    printf("p4:\n");
    for(int i=0;i<colour_number;i++) {
        printf("%d\t%d\t%d\n", symbol->palette[3*colour_number*3+i*3], symbol->palette[3*colour_number*3+i*3+1], symbol->palette[3*colour_number*3+i*3+2]);
    }
#endif // TEST_MODE
...
#if TEST_MODE
    FILE *fp;
    printf("wc:%d, wr:%d, Pg:%d, Pn: %d", wc, wr, Pg, Pn);
 ...
 */
Int32 decodeSampledMaster(Bitmap* matrix, J_Decoded_Symbol* symbol) {
    if(matrix == NULL) {
    ...
    if (readColourPaletteInMaster(matrix, symbol, data_map, &module_count, &x, &y) < 0) {
        reportError("Reading colour palettes in master symbol failed");
        return FAILURE;
    }
    ...
    Float norm_palette[colour_number * 4 * COLOR_PALETTE_NUMBER]; // 整幅图共有4个色盘
...
J_Data* decodeData(J_Data* bits) {
    ...
    jab_encode_mode mode = Upper;   // 跟你们官方文档不一样阿？
...
// 拧为啥要用marix？？？？？？？？？？
Boolean decodeMaster(J_Found_Symbol* found_ms, J_Decoded_Symbol* decoded_master_symbol) {
    if (!found_ms) {
        return FAILURE;
    } else {
        ...
        Int32 decode_result = decodeSampledMaster(found_ms->sampled_img, decoded_master_symbol); // 解码结果
        ...
        // 若只依靠 finder patterns 不可解码, 尝试介入 alignment patterns ？？？？？？？？是否需要
            ...
            matrix = sampleSymbolByAlignmentPattern(matrix, ch, decoded_master_symbol, found_ms->fps);
            clean_foundSymbol(found_ms);
            if(matrix == NULL) {
#if TEST_MODE
            ...
            // 现在尝试解码
            decode_result = decodeSampledMaster(matrix, decoded_master_symbol);
            free(matrix);
            if(decode_result == SUCCESS)
                return SUCCESS;
            else
...
J_Data* decodeJCodeEx_using_found_symbol(J_Found_Symbol* found_ms,
    ...
    // ---探测及解析Master区域---
    if (decodeMaster(found_ms, symbols)) {
        total++;
    }
    ...
    // ---连接解码结果---
    // 计算所有symbol的data的总长度
    Int32 total_data_length = 0;
    for(Int32 i = 0; i < total; i++) {
        total_data_length += symbols[i].data->length;
    }
    // 分配一个存放解码结果的区域（一个可变结构体）
    J_Data* decoded_bits = (J_Data *)malloc(sizeof(J_Data) + total_data_length * sizeof(Char));
    if(decoded_bits == NULL) {
        reportError("Memory allocation for decoded bits failed");
        if(status) *status = 1; // 解码失败
        return NULL;
    }
    // 拼合Master的比特流
    Int32 offset = 0; // 当前，在数据段decoded_bits->data对于其首地址的偏移地址
    for(Int32 i = 0; i < total; i++) {
        Char* src = symbols[i].data->data;
        Char* dst = decoded_bits->data;
        dst += offset;
    ...
    // ---根据比特流译码---
    decoded_bits->length = total_data_length; // 已翻译的数据
...
/**
 ...
 @return 该码块数据流
 */
J_Data* decodeJCodeEx(const char src[], Int32 mode, Int32* status, J_Decoded_Symbol* symbols) {
    if(status) *status = 0;
    if(!symbols) {
        reportError("Invalid symbol buffer");
        return NULL;
    ...
    return decodeJCodeEx_using_found_symbol(found_ms, mode, status, symbols);
}
...
/**
 ...
 @param mode NORMAL_DECODE: only output completely decoded data when all symbols are correctly decoded | COMPATIBLE_DECODE: also output partly decoded data even if some symbols are not correctly decoded
 @param status 解码标志位 (0: not detectable, 1: not decodable, 2: partly decoded with COMPATIBLE_DECODE mode, 3: fully decoded)
 @return 该码块数据流 | NULL if failed
 */
J_Data* decodeJCode(const char src[], Int32 mode, Int32* status) {
    J_Decoded_Symbol symbols[MAX_SYMBOL_NUMBER];
    return decodeJCodeEx(src, mode, status, symbols);
}
