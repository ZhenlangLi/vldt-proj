 ...
 * adapted and optimized by Han Li from libjabcode
...
#define MAX_SYMBOL_NUMBER       1              // Han : Before: 61
...
typedef struct {
	...
	J_Symbol*	symbols;				///< Pointer to internal representation of JAB Code symbols
	Bitmap*		bitmap;
} J_Encode;
...
extern Int32 generateJABCode(J_Encode* enc, J_Data* data);
    // 应当封装为一个函数
...
//extern Boolean saveImageCMYK(Bitmap* bitmap, Boolean isCMYK, Char* filename);
//extern Bitmap* readImage(Char* filename);

// Error & Info
extern void reportError(Char* message);
extern void reportInfo(Char* message);
